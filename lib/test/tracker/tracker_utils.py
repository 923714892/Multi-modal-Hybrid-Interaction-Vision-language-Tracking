...
def vis_attn_maps(attn_weights, q_w, k_w, skip_len, x1, x2, x1_title, x2_title, save_path='.', idxs=None):
    ...
    attn_weights_mean = attn_weights[0]
    attn_weights_mean = torch.sum(attn_weights_mean,dim=-1).view(16,16)
    min = torch.min(attn_weights_mean)
    max = torch.max(attn_weights_mean)
    attn_weights_mean  = (attn_weights_mean - min)/(max-min)
    attn_weights_mean = attn_weights_mean.unsqueeze(dim=0).cpu()
    # attn_weights_mean = torch.relu(attn_weights_mean)
    ...
    for attn_weight in attn_weights_mean:
        ...
        # min = torch.min(attn_weight)
        # max = torch.max(attn_weight)
        # attn_weight  = (attn_weight - min)/(max-min)
        ax.imshow(attn_weight, cmap='cividis', interpolation='bilinear')
        #cazhihuanhuan
        ax.axis('off')
